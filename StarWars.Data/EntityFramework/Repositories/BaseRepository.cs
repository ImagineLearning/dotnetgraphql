...
namespace StarWars.Data.EntityFramework.Repositories
{
        ...
        where TEntity : class, IEntity<TKey>, new()
    {
        ...
        protected BaseRepository(DbContext db, IElasticClient client, ILogger logger)
        {
            _db = db;
			_client = client;
            _logger = logger;
        ...
        public virtual Task<List<TEntity>> GetAll()
        {
            ...
            return _client.SearchAsync<TEntity>(s => s.AllIndices().Query(q => q.Match(qs => qs.Query("")))).ContinueWith(x => x.Result.Documents.ToList());
        ...
        public Task<List<TEntity>> GetAll(string include)
        {
            ...
            return _client.SearchAsync<TEntity>(s => s.AllIndices().Query(q => q.Match(m => m.Query("")))).ContinueWith(x => x.Result.Documents.ToList());
        ...
        public virtual Task<TEntity> Get(TKey id)
        {
			...
			return _client.SearchAsync<TEntity>(s => s.AllIndices().Query(q => q.Match(m => m.Field(f => f.Id).Query(id.ToString())))).ContinueWith(x => x.Result.Documents.ToList().FirstOrDefault());
